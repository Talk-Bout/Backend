...
CREATE TABLE `Star` (
    ...
    UNIQUE INDEX `Star.bootcampName_nickname_unique`(`bootcampName`, `nickname`),
    PRIMARY KEY (`starId`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `Review` (
    ...
    `bootCampBootcampName` VARCHAR(191),
...
-- AddForeignKey
ALTER TABLE `Answer` ADD FOREIGN KEY (`questionQuestionId`) REFERENCES `Question`(`questionId`) ON DELETE SET NULL ON UPDATE CASCADE;
...
-- AddForeignKey
ALTER TABLE `Star` ADD FOREIGN KEY (`nickname`) REFERENCES `User`(`nickname`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Star` ADD FOREIGN KEY (`bootcampName`) REFERENCES `BootCamp`(`bootcampName`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Review` ADD FOREIGN KEY (`nickname`) REFERENCES `User`(`nickname`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Review` ADD FOREIGN KEY (`bootCampBootcampName`) REFERENCES `BootCamp`(`bootcampName`) ON DELETE SET NULL ON UPDATE CASCADE;
